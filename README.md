...
이 워크스페이스의 심장은 **하루의 순환**입니다: 흩어진 채널의 정보가 자동으로 모이고 → AI가 갈라서 제자리에 두고 → 아침 상태판이 밀린 것과 이상한 것을 보여줍니다. 명령어를 외울 필요 없이 한국어로 말하면 됩니다:

```
morning                        # 아침 상태판 + 데일리 노트 + 밀린 분류까지 이어감
수집 돌려줘                    # 연결된 채널에서 지금 당겨오기
인박스 분류해줘                # 쌓인 수집분 판정 (승인은 당신)
회신 써줘: [상대 원문]         # 회신 초안 전문 생성 — 발송은 "보내줘" 후에만
장부 맞춰줘                    # 거래내역 ↔ 발행 대조, 미수금 정리
할 일 추가해줘: 이메일 답장    # todo 스킬
```

**처음이라면 → [`00-system/03-guides/일-순환-사용법.md`](00-system/03-guides/일-순환-사용법.md)** — 클로드코드가 처음인 분 기준으로, 아침에 뭐라고 말하고 상태판을 어떻게 읽고 언제 빈칸을 채우는지 장면으로 설명합니다.
